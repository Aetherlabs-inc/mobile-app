    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const profileData: any = {
                ...
                name: form.fullName,
                full_name: form.fullName,
                // Try to use new fields (if migration has been run)
                user_type: userType,
                location: location,
            ...
            // Add role-specific fields (these exist in the base schema)
            if (form.role === 'artist' && form.orgName) {
                profileData.studio_name = form.orgName;
            } else if (form.role === 'gallery' && form.orgName) {
                profileData.gallery_info = form.orgName;
            }

            // Step 4: Create/update profile in profiles table
            // Try with all fields first, fallback to base fields if columns don't exist
            ...
            if (fullProfileError) {
                console.log('Full profile error (may be missing columns):', fullProfileError.message);
                // Try with only base fields that exist in the schema
                const baseProfileData = {
                    ...
                    gallery_info: profileData.gallery_info || null,
                };
